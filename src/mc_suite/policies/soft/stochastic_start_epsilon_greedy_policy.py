...
class StochasticStartEpsilonGreedyPolicy(BaseSoftPolicy):
    ...
    def get_action_probs(self, state, action):
        policy_act = self.policy_dict[state]

        action_probs = 1.0 - self.epsilon if action == policy_act else self.epsilon
    ...
    def get_action_deterministic(self, state, deterministic: bool = False):
        if deterministic:
